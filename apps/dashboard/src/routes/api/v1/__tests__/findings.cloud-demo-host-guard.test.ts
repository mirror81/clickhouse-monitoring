/**
 * #2172 — route-level regression: GET /api/v1/findings must reject a
...
const mockFetchData = mock(async () => ({ data: [], error: null }))
...
function get(host: string): Promise<Response> {
  return handler({
    request: new Request(`http://x/api/v1/findings?host=${host}`),
  })
}

describe('GET /api/v1/findings — cloud demo-host guard (#2172)', () => {
  beforeEach(() => {
    cloudMode = false
    signedIn = false
  ...
  test('OSS: authenticated caller + host=0 is unaffected (reaches ClickHouse)', async () => {
    cloudMode = false
    signedIn = true
  ...
  test('anonymous cloud: host=0 is unaffected (reaches ClickHouse)', async () => {
    cloudMode = true
    signedIn = false
    const res = await get('0')
    expect(res.status).toBe(200)
