/**
 * #2172 — route-level regression: GET /api/v1/host-status must reject a
 ...
 * of the underlying boolean logic, and charts/__tests__/cloud-demo-host-guard.test.ts
...
function get(hostId: string): Promise<Response> {
  return handler({
    request: new Request(`http://x/api/v1/host-status?hostId=${hostId}`),
  })
}

describe('GET /api/v1/host-status — cloud demo-host guard (#2172)', () => {
  beforeEach(() => {
    cloudMode = false
    signedIn = false
  ...
  test('authenticated cloud + hostId=0: rejected with structured empty response', async () => {
    ...
    expect(body.data).toEqual({ version: '', uptime: '', hostname: '' })
