/**
 * #2172 — route-level regression: GET /api/v1/explorer/columns must reject a
...
const { Route } = await import('../columns')
const handler = getGetHandler(Route)

function get(hostId: string): Promise<Response> {
  return handler({
    request: new Request(
      `http://x/api/v1/explorer/columns?hostId=${hostId}&database=default&table=t`
    ),
  })
}

describe('GET /api/v1/explorer/columns — cloud demo-host guard (#2172)', () => {
  beforeEach(() => {
    cloudMode = false
    signedIn = false
