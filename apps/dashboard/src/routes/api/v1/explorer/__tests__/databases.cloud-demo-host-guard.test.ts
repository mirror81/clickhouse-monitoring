/**
 * #2172 — route-level regression: GET /api/v1/explorer/databases must reject a
...
const { Route } = await import('../databases')
const handler = getGetHandler(Route)

function get(hostId: string): Promise<Response> {
  return handler({
    request: new Request(`http://x/api/v1/explorer/databases?hostId=${hostId}`),
  })
}

describe('GET /api/v1/explorer/databases — cloud demo-host guard (#2172)', () => {
  beforeEach(() => {
    cloudMode = false
    signedIn = false
