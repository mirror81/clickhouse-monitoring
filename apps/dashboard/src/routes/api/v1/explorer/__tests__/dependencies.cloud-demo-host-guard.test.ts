/**
 * #2172 — route-level regression: GET /api/v1/explorer/dependencies must
...
const { Route } = await import('../dependencies')
const handler = getGetHandler(Route)

function get(hostId: string): Promise<Response> {
  return handler({
    request: new Request(
      `http://x/api/v1/explorer/dependencies?hostId=${hostId}&database=default&table=t`
    ),
  })
}

describe('GET /api/v1/explorer/dependencies — cloud demo-host guard (#2172)', () => {
  beforeEach(() => {
    cloudMode = false
    signedIn = false
