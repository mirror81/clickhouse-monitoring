/**
 * #2172 — route-level regression: GET /api/v1/explorer/indexes must reject a
...
const { Route } = await import('../indexes')
const handler = getGetHandler(Route)

function get(hostId: string): Promise<Response> {
  return handler({
    request: new Request(`http://x/api/v1/explorer/indexes?hostId=${hostId}`),
  })
}

describe('GET /api/v1/explorer/indexes — cloud demo-host guard (#2172)', () => {
  beforeEach(() => {
    cloudMode = false
    signedIn = false
