/**
 * #2172 — route-level regression: GET /api/v1/explorer/preview must reject a
...
function get(hostId: string): Promise<Response> {
  return handler({
    request: new Request(
      `http://x/api/v1/explorer/preview?hostId=${hostId}&database=default&table=t`
    ),
  })
}

describe('GET /api/v1/explorer/preview — cloud demo-host guard (#2172)', () => {
  beforeEach(() => {
    cloudMode = false
    signedIn = false
