/**
 * #2172 — route-level regression: GET /api/v1/explorer/query-log must reject
...
const { Route } = await import('../query-log')
const handler = getGetHandler(Route)

function get(hostId: string): Promise<Response> {
  return handler({
    request: new Request(
      `http://x/api/v1/explorer/query-log?hostId=${hostId}&queryId=abc`
    ),
  })
}

describe('GET /api/v1/explorer/query-log — cloud demo-host guard (#2172)', () => {
  beforeEach(() => {
    cloudMode = false
    signedIn = false
  ...
  test('authenticated cloud + hostId=0: rejected with structured empty response', async () => {
    ...
    const body = (await res.json()) as {
      success: boolean
      data: unknown
      metadata: { found: boolean; unavailable: { reason: string } }
    }
